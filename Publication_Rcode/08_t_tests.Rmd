...
table <- read.csv(here::here("Publication_Data","Collars","Distances_During_Fire_Final.csv"))
firedeer <- table[-c(2,6,7,12, 15),]
mean(firedeer$Distance)#avg = 1256.931, min =263.5014, max=4401

nonfire <- table[c(2,6,7,12, 15),]
mean(nonfire$Distance) #293.3645

fire <- as.numeric(firedeer$Distance)
nfire <- as.numeric(nonfire$Distance)

#not equal variances use welches t test
var.test(firedeer$Distance, nonfire$Distance)

t.test(firedeer$Distance, nonfire$Distance, var.equal = FALSE)
