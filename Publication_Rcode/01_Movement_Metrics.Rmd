...
collars <- collars[complete.cases(collars),] #remove any cases that were not complete (had NAs or were vacant)
...
collars <- read.csv(here::here('Publication_Data','Collars','deer-gps-postfirebuck.csv')) #load in datafram
collars <- collars[complete.cases(collars),] #remove any cases that were not complete (had NAs or were vacant)
collars$TimeStamp <- as.POSIXct((collars$TimeStamp), format = "%Y-%m-%d %H:%M:%S", origin = '1970-01-01', tz = "UTC") #Create date object from time stamp
